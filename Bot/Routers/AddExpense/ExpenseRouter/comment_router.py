...
def create_comment_router(bot: ProjectBot):
    ...
    async def set_comment(message: Message, state: FSMContext):
        ...
        if wallet == "Проект":
            ...
            category_name = data.get("category_name", bot.google_sheets.get_category_name(chapter_code, category_code))
            ...
            processing_message = await bot.send_message(chat_id=chat_id, text="Идет процесс добавления записи о долге и расходе...")
            ...
            bot.google_sheets.update_expense_with_comment(chapter_code, category_code_to_use, date, borrowing_amount, comment)
            bot.google_sheets.record_borrowing(creditor, date, amount, comment)

            # Записываем операцию в БД
            operation_id = await bot.record_operation(date, creditor, chapter_code, category_code_to_use, amount, coefficient, comment)
