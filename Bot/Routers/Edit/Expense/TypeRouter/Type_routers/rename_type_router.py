...
from Bot.Keyboards.Edit.type import RenameTypeCallback, CancelTypeRenameCallback, create_edit_type_kb, \
...
def create_rename_type_router():
    rename_type_router = Router()
    ...
    @rename_type_router.callback_query(RenameTypeCallback.filter(F.operation == "expense"))
    ...
    @rename_type_router.callback_query(RenameExpenseTypeCallback.new_type_name,
    ...
    rename_type_router.message.middleware(LimitTypeLenMiddleware())

    @rename_type_router.message(RenameExpenseTypeCallback.new_type_name, flags={"limit_len": True})
    async def rename_type(message: Message, state: FSMContext, bot: Bot):
        ...
        try:
            ...
            await bot.edit_message_text(chat_id=chat_id, message_id=query_message,
                                        text=f'Выберите действие по изменению типа "{type_name}" в категории "{category_name}":',
                                        reply_markup=create_edit_type_kb(category_id, type_id, ExpenseType))

    return rename_type_router
